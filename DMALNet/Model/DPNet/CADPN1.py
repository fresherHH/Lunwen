...
import os
from Model.DPNet.SRMConv2d import SRMConv2dFilter
...
class Attention(nn.Module):
    def __init__(self, in_dims, out_dim=None, add=True):
        ...
        self.in_dims = in_dims // 4

        if out_dim is None:
            out_dim = in_dims // 4
        self.out_dim = out_dim
        self.add = add
        self.att3 = Self_Attn(832)
        self.att2 = Self_Attn(832)
        self.att1 = Self_Attn(832)
        self.att0 = Self_Attn(832)
    ...
    def forward(self,x):
        ...
        x1 = x[:, 2 * self.in_dims: 3 * self.in_dims, :, :]
        x0 = x[:, 3*self.in_dims:, :, :]

        m_batchsize, C, width, height = x1.size()

        q0 = self.att1.query_conv(x0).view(
            m_batchsize, -1, width * height).permute(0, 2, 1)  # B X C X(N)
        k0 = self.att1.query_conv(x0).view(
            m_batchsize, -1, width * height)  # B X C x (*W*H)
        v0 = self.att1.value_conv(x0).view(
            m_batchsize, -1, width * height)  # B X C X N
        ...
        att00 = self.softmax(torch.bmm(q0, k0))
        out00 = torch.bmm(v0, att00.permute(0, 2, 1))
        out00 = out00.view(m_batchsize, self.out_dim, width, height)
        att01 = self.softmax(torch.bmm(q0, k1))
        out01 = torch.bmm(v0, att01.permute(0, 2, 1))
        out01 = out01.view(m_batchsize, self.out_dim, width, height)
        att02 = self.softmax(torch.bmm(q0, k2))
        out02 = torch.bmm(v0, att02.permute(0, 2, 1))
        out02 = out02.view(m_batchsize, self.out_dim, width, height)
        att03 = self.softmax(torch.bmm(q0, k3))
        out03 = torch.bmm(v0, att03.permute(0, 2, 1))
        out03 = out03.view(m_batchsize, self.out_dim, width, height)
        out0 = out00 + out01 + out02 + out03


        att10 = self.softmax(torch.bmm(q1, k0))
        out10 = torch.bmm(v1, att10.permute(0, 2, 1))
        out10 = out10.view(m_batchsize, self.out_dim, width, height)
        ...
        out1 = out10 + out11 + out12 + out13

        att20 = self.softmax(torch.bmm(q2, k0))
        out20 = torch.bmm(v2, att20.permute(0, 2, 1))
        out20 = out20.view(m_batchsize, self.out_dim, width, height)
        ...
        out2 =  out20 + out21 + out22 + out23

        att30 = self.softmax(torch.bmm(q3, k0))
        out30 = torch.bmm(v3, att30.permute(0, 2, 1))
        out30 = out30.view(m_batchsize, self.out_dim, width, height)
        ...
        if self.add:
            x0 = self.gamma0 * out0 + x0
            x1 = self.gamma1 * out1 + x1
            x2 = self.gamma2 * out2 + x2
            x3 = self.gamma3 * out3 + x3
        else:
            x0 = self.gamma0 * out0
            x1 = self.gamma1 * out1
            x2 = self.gamma2 * out2
...
class TranModelatten(nn.Module):
    def __init__(self, num_out_classes=2):
        super(TranModelatten, self).__init__()
...
class Attentionv1(nn.Module):
    def __init__(self, in_dims, out_dim=None, add=True):
        ...
        self.out_dim = out_dim
        self.add = add
        self.att3 = Self_Attn(704)
        self.att2 = Self_Attn(704)
        self.att1 = Self_Attn(704)
...
class TranModelattenv1(nn.Module):
    def __init__(self, num_out_classes=2):
        super(TranModelattenv1, self).__init__()
...
if __name__=="__main__":
    ...
    # model = dpn68()
    model = TranModelattenv1()
    # model = Attention(in_dims=3328)
    print(model)
