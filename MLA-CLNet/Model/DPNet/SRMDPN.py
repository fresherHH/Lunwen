...
import os
from Model.DPNet.SRMConv2d import SRMConv2dFilter, SRMConv2d
...
class SRMDPNv1(nn.Module):
    def __init__(self, num_out_classes=2):
        super(SRMDPNv1, self).__init__()
        ...
        self.down144 = nn.Sequential(
            SRMConv2dFilter(inc=144, outc=144),
...
class SRMDPNv2(nn.Module):
    def __init__(self, num_out_classes=2):
        super(SRMDPNv2, self).__init__()
        ...
        self.conv5_bn_ac = model.features.conv5_bn_ac
        self.classifier = model.classifier
        self.atten2_3 = Attentionv2(640)
        self.atten3_4 = Attentionv2(1408)
        self.atten4_5 = Attentionv2(1664)
    ...
    def forward(self, x):
        ...
        x32 = self.down320(x32)


        x = self.fea_conv4x(x)
        x4 = torch.cat([x[0], x[1]], dim=1)
        x43 = torch.cat([x4, x32], dim=1)
...
class SRMDPNv3(nn.Module):
    def __init__(self, num_out_classes=2):
        super(SRMDPNv3, self).__init__()
    ...
    def forward(self, x):
        ...
        x = self.fea_conv1x(x)   # input_size: (3, 224, 244) output_size: (10, 56, 56)
        # print(x.size())



        x = self.fea_conv2x(x)
        x2 = torch.cat([x[0], x[1]], dim=1)  # input_size: (10, 56, 56) output_size: (144, 56, 56)

        x2 = self.down144(x2)       # input_size: (144, 56, 56) output_size: (320, 28, 28)
        # print(x[0].size())
        # print(x[1].size())


        x = self.fea_conv3x(x)
        x3 = torch.cat([x[0], x[1]], dim=1)    # input_size: (144, 56, 56) output_size: (320, 28, 28)
        x32 = torch.cat([x3, x2], dim=1)       # (640, 28, 228)
        x32 = self.atten2_3(x32)               # (320, 28, 28)

        x3 = self.down320(x32)                # (704, 14, 14)
        # print(x32.size())
        # print(x[0].size())
        # print(x[1].size())

        x = self.fea_conv4x(x32)
        x4 = torch.cat([x[0], x[1]], dim=1)    # input_size: (320, 28, 28) output_size: (704, 14, 14)
        # print(x4.size())
...
class SRMDPNv4(nn.Module):
    def __init__(self, num_out_classes=2, dropout=0.5,
                 weight_norm=False,):
        super(SRMDPNv4, self).__init__()
        ...
        if weight_norm:
            ...
            self.last_linear = nn.Sequential(
                ...
                nn.utils.weight_norm(
                    nn.Linear(832 * 7 * 7, num_out_classes), name='weight')
            )

        self.last_linear = nn.Sequential(
            nn.Dropout(p=dropout),
            nn.Linear(832 * 7 * 7, num_out_classes)
        )
